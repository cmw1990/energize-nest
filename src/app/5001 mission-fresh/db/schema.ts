...
/**
 * Database helpers to ensure tables exist
 */
export const createTablesIfNotExist = async () => {
  try {
    // Check and create settings table
    const { error: settingsError } = await supabase.from('quit_smoking_settings').select('id').limit(1);
    if (settingsError && settingsError.code === '42P01') {
      console.log('Creating quit_smoking_settings table');
      // Code to create table would go here in a real migration
    }
