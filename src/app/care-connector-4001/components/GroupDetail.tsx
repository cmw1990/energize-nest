...
export const GroupDetail: React.FC<GroupDetailProps> = ({ session }) => {
  ...
  const loadGroupData = useCallback(async () => {
    ...
    loadingTimeoutRef.current = setTimeout(() => {
      ...
      console.error('Loading timeout triggered');
    ...
    try {
      ...
      const { data: groupData, error: groupError } = await careConnector.groups.getGroupDetails(groupId as string);
      ...
      if (groupError) {
        ...
        return; // Stop loading other data if group fetch fails
      } 
      ...
      if (!groupData || groupData.length === 0) {
        ...
        return; // Stop loading other data if group not found
      }
      ...
      const [
        ...
        careConnector.members.getGroupMembers(groupId as string),
        careConnector.tasks.getGroupTasks(groupId as string),
        careConnector.events.getGroupEvents(groupId as string),
        careConnector.posts.getGroupPosts(groupId as string)
      ]);

      // Set data from results (with error handling)
      if (membersError) {
            ...
            {(groupData.user_role === 'owner' || 
              ...
              <Button onClick={() => {
                ...
                (async () => {
                  try {
                    setIsLoading(true);
                    const { data, error } = await careConnector.events.createEvent(
                      groupId as string,
                      eventTitle,
                      eventDesc || '',
                          ...
                          onClick={() => {
                            ...
                            if (window.confirm(`Are you sure you want to delete event "${event.title}"?`)) {
                              (async () => {
                                try {
                                  setIsLoading(true);
                                  const { error } = await careConnector.events.deleteEvent(event.id);
                    ...
                    (async () => {
                      try {
                        ...
                        const { data, error } = await careConnector.events.createEvent(
                          groupId as string,
                          "Group Meeting",
                          "Weekly team check-in and planning session",
            ...
            {(groupData.user_role === 'owner' || 
              ...
              <Button onClick={() => {
                ...
                (async () => {
                  try {
                    setIsLoading(true);
                    const { data, error } = await careConnector.tasks.createTask(
                      groupId as string,
                      taskTitle,
                      taskDesc || '',
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              ...
                              (async () => {
                                try {
                                  setIsLoading(true);
                                  const { error } = await careConnector.tasks.updateTaskStatus(task.id, nextStatus);
                            ...
                            onClick={() => {
                              ...
                              if (window.confirm(`Are you sure you want to delete task "${task.title}"?`)) {
                                (async () => {
                                  try {
                                    setIsLoading(true);
                                    const { error } = await careConnector.tasks.deleteTask(task.id);
                    ...
                    (async () => {
                      try {
                        ...
                        for (const task of sampleTasks) {
                          await careConnector.tasks.createTask(
                            groupId as string,
                            task.title,
                            task.description,
                            task.due_date,
            ...
            {(groupData.user_role === 'owner' || 
              ...
              <Button onClick={() => {
                ...
                (async () => {
                  try {
                    setIsLoading(true);
                    const { data, error } = await careConnector.posts.createPost(
                      groupId as string,
                      postContent
                    );
                          ...
                          onClick={() => {
                            ...
                            if (window.confirm("Are you sure you want to delete this post?")) {
                              (async () => {
                                try {
                                  setIsLoading(true);
                                  const { error } = await careConnector.posts.deletePost(post.id);
                      ...
                      <Button 
                        ...
                        onClick={() => {
                          ...
                          (async () => {
                            try {
                              setIsLoading(true);
                              const { error } = await careConnector.posts.addComment(post.id, comment);
                    ...
                    (async () => {
                      try {
                        ...
                        for (const content of samplePosts) {
                          await careConnector.posts.createPost(
                            groupId as string,
                            content
                          );
                        }
