...
import { Search, Filter, MapPin, Clock, Star, UserCheck, ChevronDown, Loader2 } from 'lucide-react';
...
import { caregiverConnectorApi } from '@/api/care8ConnectorApi';
import { Alert, AlertDescription } from '@/components/ui/alert';

interface CaregiverConnectorProps {
  session: Session | null;
}

export const CaregiverConnector: React.FC<CaregiverConnectorProps> = ({ session }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [location, setLocation] = useState('');
  const [showFilters, setShowFilters] = useState(false);
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [sortBy, setSortBy] = useState('highest-rated');
  const [caregiverType, setCaregiverType] = useState('all');
  ...
  // Function to fetch caregivers from API
  const fetchCaregivers = async () => {
    setLoading(true);
    setError(null);
    
    try {
      const response = await caregiverConnectorApi.getCaregivers();
      
      if (response.error) {
        throw new Error(response.error.message || 'Failed to fetch caregivers');
      }
      
      // Sort the caregivers based on sortBy value
      let sortedCaregivers = [...response.data];
      
      if (sortBy === 'highest-rated') {
        sortedCaregivers.sort((a, b) => b.rating - a.rating);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (caregiverType !== 'all') {
        filters.specialties = caregiverType;
      }
      ...
      // Fetch caregivers with filters
      const response = await caregiverConnectorApi.getCaregivers(filters);
      ...
      let filteredCaregivers = response.data;
      
      if (searchTerm) {
        const searchLower = searchTerm.toLowerCase();
        filteredCaregivers = filteredCaregivers.filter((caregiver: any) => 
          caregiver.name.toLowerCase().includes(searchLower) ||
          caregiver.bio.toLowerCase().includes(searchLower) ||
          caregiver.specialties.some((specialty: string) => 
      ...
      // Sort the caregivers based on sortBy value
      let sortedCaregivers = [...filteredCaregivers];
      ...
      setError(err.message || 'Failed to search caregivers');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Caregiver Connector</h1>
        <p className="text-gray-600 dark:text-gray-400">Find experienced caregivers for your specific needs</p>
      </div>
          ...
          <Card className="mb-4">
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-medium mb-2">Caregiver Type</label>
                ...
                <div>
                  ...
                  <Select value={sortBy} onValueChange={(value) => {
                    setSortBy(value);
                    // Re-sort caregivers when sort option changes
      ...
      {!loading && caregivers.length === 0 && (
        <div className="text-center py-12">
          <h3 className="text-xl font-medium mb-2">No caregivers found</h3>
                  ...
                  <div className="p-6">
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <div className="flex items-center mt-1">
                          ...
                          <span className="text-sm text-gray-500 ml-1">({caregiver.review_count} reviews)</span>
                        </div>
                    ...
                    <div className="mt-4">
                      <div className="flex flex-wrap gap-1 mb-3">
                        {caregiver.specialties && caregiver.specialties.map((specialty: string) => (
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium">{caregiver.hourly_rate}/hr</p>
...
export default CaregiverConnector; 