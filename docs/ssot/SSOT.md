# Well-Charged: Single Source of Truth (SSOT)
...
8. [Frequently Met Errors](#frequently-met-errors)
...
1. **No Frontend Mock Data**: Never use hardcoded mock data in the frontend. All data displayed in the application must come from Supabase tables.
   
2. **Data Initialization**: If demo or sample data is needed for testing or development:
   - Create proper tables in Supabase
   - Use migration scripts to populate tables with initial data
   - Document any seed data in SQL files under `src/integrations/supabase/`
   
3. **Error Handling**: When database connections fail:
   - Display appropriate error messages to users
   - Log detailed errors for debugging
   - Implement graceful UI fallbacks that prompt users to try again
   - Never fall back to hardcoded mock data as this creates inconsistency between displayed and stored data
   
4. **Empty States**: Design proper empty state UI components that guide users to create data instead of showing fake data
...
## Frequently Met Errors

This section documents common errors encountered during development and their solutions to help troubleshoot issues quickly.

### Environment Variables

#### Error: "process is not defined"

**Description**: When running the application in Vite, you may encounter a `"Uncaught ReferenceError: process is not defined"` error. This usually happens when trying to access environment variables using `process.env` in client-side code.

**Cause**: Vite uses a different method for accessing environment variables than Node.js. Unlike Node.js applications that use `process.env`, Vite provides environment variables through `import.meta.env`.

**Solution**: 
1. Replace all instances of `process.env.VARIABLE_NAME` with `import.meta.env.VARIABLE_NAME`.

**Example**:
```typescript
// Incorrect (causes "process is not defined" error)
const supabaseUrl = process.env.VITE_SUPABASE_URL || 'default-url';

// Correct approach for Vite applications
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || 'default-url';
```

**Note**: All environment variables must be prefixed with `VITE_` to be exposed to the client-side code. For example, `VITE_SUPABASE_URL` instead of just `SUPABASE_URL`.

### TypeScript Errors

#### Error: "Property does not exist on type 'never'"

**Description**: When working with Supabase client responses, you may encounter TypeScript errors like `"Property 'care_groups' does not exist on type 'never'"`.

**Cause**: These errors often occur due to type definition mismatches between your code and the actual Supabase database schema. The TypeScript compiler doesn't recognize the structure of the data returned from Supabase queries.

**Solution**:
1. Regenerate the TypeScript types from your Supabase schema using:
   ```bash
   npm run supabase:generate-types
   ```
2. If the issue persists, you may need to use type assertions to help TypeScript understand the shape of your data:
   ```typescript
   // Add type assertion to help TypeScript understand the structure
   const formattedMyGroups = (myGroupsData as any[])?.map(item => ({
     ...item.care_groups,
     role: item.role
   }));
   ```
...
#### Error: "Missing PostgreSQL connection string"

**Description**: When running database migration scripts, you might see errors related to missing PostgreSQL connection strings.

**Cause**: These issues typically occur when environment variables are not properly set or accessible to the script.

**Solution**:
1. Ensure your `.env` file contains the necessary connection string:
   ```
   POSTGRES_CONNECTION_STRING=postgresql://postgres.[ref]:[password]@[host]:[port]/postgres
   ```
2. Make sure the script loads environment variables correctly, usually at the top of the file:
   ```javascript
...
**Description**: Components fail to render with errors like "Cannot read properties of undefined (reading 'map')" or similar.

**Cause**: These errors typically occur when trying to access properties of objects or arrays that are still undefined or null during initial render.

**Solution**:
1. Add null checks and fallbacks:
   ```jsx
   // Before (error-prone)
   {data.items.map(item => <div>{item.name}</div>)}
   
   // After (safe)
   {data?.items?.map(item => <div>{item?.name || 'Unnamed'}</div>) || <div>No items available</div>}
   ```
2. Ensure loading states are properly handled:
   ```jsx
