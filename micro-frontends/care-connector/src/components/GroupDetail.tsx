...
import { getGroupById, getGroupMembers, getGroupTasks, getGroupEvents, getGroupPosts, ExtendedGroupMember } from '../lib/careConnectorApiClient';
            ...
            {(groupData.user_role === 'owner' || 
              ...
              <Button onClick={() => {
                ...
                (async () => {
                  try {
                    setIsLoading(true);
                    const { data, error } = await getGroupTasks(groupId as string, taskTitle, taskDesc || '', new Date(dueDate).toISOString(), assigneeId || '');
                            ...
                            onClick={() => {
                              ...
                              if (window.confirm(`Are you sure you want to delete task "${task.title}"?`)) {
                                (async () => {
                                  try {
                                    setIsLoading(true);
                                    const { error } = await getGroupTasks(groupId as string, '', '', '', '', '', '', task.id);
                    ...
                    (async () => {
                      try {
                        ...
                        for (const task of sampleTasks) {
                          await getGroupTasks(groupId as string, task.title, task.description, task.due_date, session?.user?.id || '');
                        }
...
const handleUpdateTaskStatus = async (taskId: string, newStatus: 'pending' | 'in_progress' | 'completed' | 'cancelled') => {
  try {
    setIsLoading(true);
    const { error } = await getGroupTasks(
      groupId as string, 
      undefined, 
      undefined, 
      undefined, 
    ...
    if (error) {
      ...
      // Refresh tasks
      const { data } = await getGroupTasks(groupId as string);
