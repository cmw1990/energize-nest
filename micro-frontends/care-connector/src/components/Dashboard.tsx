...
// Import from our careConnectorApiClient instead
import { getGroups, getTasks, Group, Task } from '../lib/careConnectorApiClient';
...
export const Dashboard: React.FC<DashboardProps> = ({ session }) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const { data: memberships, error: membershipsError } = await getGroups();
      ...
      if (memberships && memberships.length > 0) {
        for (const membership of memberships) {
          const { data: groups } = await getGroups(membership.group_id);
