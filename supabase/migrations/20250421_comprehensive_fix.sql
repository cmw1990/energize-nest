...
DROP POLICY IF EXISTS "Users can view public groups" ON public.care_groups;
DROP POLICY IF EXISTS "Group members can view their groups" ON public.care_groups;
DROP POLICY IF EXISTS "Group creators can view their created groups" ON public.care_groups;
DROP POLICY IF EXISTS "Group members can view their memberships" ON public.care_group_members;
DROP POLICY IF EXISTS "Group creators can view all memberships in their groups" ON public.care_group_members;
DROP POLICY IF EXISTS "Group creators can manage memberships" ON public.care_group_members;

-- Create minimal policies that should work
-- For care_groups: Allow all authenticated users to view public groups
CREATE POLICY "Anyone can view public groups" ON public.care_groups
    FOR SELECT USING (is_public = true);

-- For care_groups: Allow group members to view their groups
...
-- For care_groups: Allow creators to manage their groups
CREATE POLICY "Creators can manage their groups" ON public.care_groups
    FOR ALL USING (creator_id = auth.uid());

-- For care_group_members: Allow users to view their own memberships
CREATE POLICY "Users can view their own memberships" ON public.care_group_members
    FOR SELECT USING (user_id = auth.uid());

-- For care_group_members: Allow group creators to manage memberships
...
-- Create or replace the get_user_settings function to fix column reference issues
...
-- Create get_user_role function that's being called by the frontend
