...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'care_groups' AND policyname = 'Group creators can manage their groups'
    ) INTO policy_exists;
    
    IF policy_exists THEN
        EXECUTE 'DROP POLICY "Group creators can manage their groups" ON public.care_groups';
    END IF;

    SELECT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'care_groups' AND policyname = 'Users can view public groups'
    ) INTO policy_exists;
    
    IF policy_exists THEN
        EXECUTE 'DROP POLICY "Users can view public groups" ON public.care_groups';
    END IF;
